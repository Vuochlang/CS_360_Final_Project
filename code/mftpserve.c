   	...
   	servAddr.sin_family = AF_INET;
    	...
    	while(1) {
        	...
        	if (pid == 0) {  // child process
            		...
            		int hostEntry;
            		hostEntry = getnameinfo((struct sockaddr *) &clientAddr, sizeof(clientAddr), hostName, 
            		...
            		while ((readResult = read(listenFd, command, max)) > 0) {
                		if (strlen(command) > 0) {
                    			if (strncmp("Q", command, 1) == 0) {
                        			...
                        			//get a new listen fd after established a data connection
...
void commandG(int newLisFd, char* command) { //transfer the contents of <pathname> to the client with data connection fd
		...
		// read file content and write to the client using new listen fd from data connection
            	while (readResult > 0) {
...
int commandD(int listenFd) {
    	...
    	if (getsockname(socketFd2, (struct sockaddr *) &servAddr2, &length2) == -1) {
        	...
        	if (convert < 0) {
            		printf("Child %d: Failed to convert port number %d to string\n", getpid(), newPort);
        	}
        	assert(convert > 0);
